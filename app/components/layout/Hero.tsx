...
export default function Hero() {
  return (
    <section className="relative min-h-[85vh] flex items-center justify-start overflow-hidden border-b border-border w-full">
      <div
        className="absolute inset-0 -z-1 opacity-40 mix-blend-luminosity bg-cover bg-center"
        style={{
          backgroundImage:
            "url('https://images.unsplash.com/photo-1512749355846-eb142b5cc4a5?q=80&w=2070&auto=format&fit=crop')",
        }}
      />
      <div className="absolute inset-0 -z-1 bg-linear-to-b from-transparent via-(--primary-bg)/60 to-(--primary-bg) " />
      <div className="mx-auto flex max-w-400 w-full justify-between flex-col gap-4 max-md:max-w-xl max-xl:max-w-5xl max-lg:max-w-3xl px-4">
        ...
        <h1 className="uppercase flex flex-col text-(--primary-text) text-8xl font-bold max-md:text-6xl">
          The next race
          <span className="text-(--accentColor)">starts here.</span>
        </h1>
        <p className="text-xl text-(--secondary-text) font-semibold max-w-xl max-md:text-lg max-sm:md">
